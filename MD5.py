'''
origin : https://github.com/timvandermeij/md5.py
'''
...
class MD5(object):
	...
	def _step_1(cls):
		...
		cls._bit_ary =  bitarray(bit_array, endian="little")
	...
	def _step_4(cls):
		...
		for chunk_index in range(N // 16):
			...
			X = [cls._bit_ary[start + (x * 32) : start + (x * 32) + 32] for x in range(16)]
	...
	def _step_5(cls):
		...
		print(f"{format(A, '08x')}{format(B, '08x')}{format(C, '08x')}{format(D, '08x')}")
